...
enum processor {
    proc_cc, proc_cxx, proc_cpp, proc_as
};
...
static char *get_lib_dir(struct options *opts) {
    ...
    bit_suffix =
            opts->target.cpu == CPU_x86_64 || opts->target.cpu == CPU_ARM64 ? "64" : opts->target.cpu == CPU_x86_32on64
                                                                                     ? "32on64" : "32";
...
static void
build_data_lib(struct options *opts, const char *spec_file, const char *output_file, struct strarray files) {
    unsigned int i;
    struct strarray spec_args = get_winebuild_args(opts);
            ...
            switch (opts.args.str[i][1]) {
                ...
                case 'm':
                    ...
                    if (strcmp("-mcygwin", opts.args.str[i]) == 0) {
                        ...
                        if (opts.target.cpu == CPU_i386 || opts.target.cpu == CPU_x86_32on64)
