...
NTSTATUS context_to_server( context_t *to, USHORT to_machine, const void *src, USHORT from_machine )
{
    DWORD i, flags;
...
static void start_thread( void * HOSTPTR arg )
{
    ...
    struct ntdll_thread_data * HOSTPTR thread_data = (struct ntdll_thread_data *)&teb->GdiTebBatch;
    BOOL suspend;
...
NTSTATUS WINAPI NtRaiseException( EXCEPTION_RECORD *rec, CONTEXT *context, BOOL first_chance )
{
    ...
    if (first_chance) {
        call_user_exception_dispatcher( rec, context, pKiUserExceptionDispatcher );
        return STATUS_SUCCESS;
    }
