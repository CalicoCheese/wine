 ...
 */
void fpux_to_fpu( I386_FLOATING_SAVE_AREA *fpu, const XSAVE_FORMAT * HOSTPTR fpux )
{
 ...
 */
void fpu_to_fpux( XSAVE_FORMAT *fpux, const FLOATING_SAVE_AREA *fpu )
{
    unsigned int i;
...
NTSTATUS WINAPI NtSetContextThread( HANDLE handle, const CONTEXT *context )
{
    ...
    if (!self)
    {
        ...
        context_to_server( &server_context, native_machine, context, native_machine);
        ret = set_thread_context( handle, &server_context, &self, IMAGE_FILE_MACHINE_I386);
...
NTSTATUS WINAPI NtGetContextThread( HANDLE handle, CONTEXT *context )
{
    ...
    {
        context_t server_context;
        if ((ret = get_thread_context( handle, &server_context, &self, IMAGE_FILE_MACHINE_I386))) return ret;
        if ((ret = context_from_server( context, &server_context, native_machine))) return ret;
